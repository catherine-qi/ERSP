...
def get_ngrams(text_tokens: List[str], min_length=1, max_length=4) -> List[str]:
    ...
    for n in range(min_length - 1, max_length):
        ngrams = [" ".join(ngram) for ngram in zip(
            *[text_tokens[i:] for i in range(n + 1)])]
        for ngram in ngrams:
            if '#' not in ngram:
    ...
    if lemmatize:
        ...
        for i, word in enumerate(tokens):
            ...
            if pos_labels[i] in ['r']:  # For adverbs it's a bit different
                try:
                    lemma = wordnet.synset(
                        word + '.r.1').lemmas()[0].pertainyms()[0].name()
                except:
                    lemma = word
                lemmatized_words.append(lemma)
            elif pos_labels[i] in ['a', 's', 'v']:  # For adjectives and verbs
                lemmatized_words.append(
                    lemmatizer.lemmatize(word, pos=pos_labels[i]))
...
if __name__ == "__main__":
    documents = ["Information Retrieval",
                 "hi", "Natural Language Processesing"]
...
documents = ["Information Retrieval", "hi", "Natural Language Processesing"]
print(documents)
